...
# ===================== 全局变量 =====================
current_state = State()
correct_debug_msg = AttitudeTarget()
...
def data_callback(msg):
    ...
    if len(msg.data) != 6:
        ...
        return
    correct_debug_msg.header.stamp = rospy.Time.now()
    correct_debug_msg.header.frame_id = "base_link"  # 可根据需要修改
    correct_debug_msg.body_rate.x = msg.data[3]
    correct_debug_msg.body_rate.y = msg.data[4]
    correct_debug_msg.body_rate.z = msg.data[5]
    correct_debug_msg.thrust = msg.data[2]
...
def main():
    ...
    # ------------ 发布者定义 ------------
    nmpc_pub = rospy.Publisher("/mavros/setpoint_raw/attitude", AttitudeTarget, queue_size=10)
    ...
    rate = rospy.Rate(150)  # 20Hz高频发送（PX4要求≥2Hz）
    ...
    while not rospy.is_shutdown():
        ...
        rospy.loginfo_throttle(2, "%.4f",correct_debug_msg.thrust)
