...
class NMPCParams:
    def __init__(self):
        ...
        self.Np = 25   # 预测时域
        self.Nc = 15   # 控制时域
        self.Q = np.diag([60, 60, 20,
                          10, 10, 5,
                          5, 5, 2.5,
                          10, 10, 5])  # 状态权重（位置/速度/姿态/角速度）
        self.P = self.Q * 0.8  # 终端权重（更重视终端状态）
        self.R = np.diag([0.5, 0.5, 5, 30, 30, 25])  # 控制权重（推力/力矩）
...
class UAVHostController:
    ...
    def generate_reference_trajectory(self, t_current):
        ...
        roll_decay = np.linspace(current_roll, 0, Np+1)  # 逐步归零
        pitch_decay = np.linspace(current_pitch, 0, Np+1)  # 逐步归零
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            #     f"时间:{t_current:.2f}s | "
            #     f"位置:{self.x_current[0:3].round(3)} | "
            #     f"求解:{'成功' if success else '失败'} | "
            #     f"耗时:{solve_time*1000:.2f}ms"
            # )
            # rospy.loginfo_throttle(1.0, f"初始状态x0:{np.array2string(self.controller.ocp.constraints.x0, precision=3, floatmode='fixed', suppress_small=True)}")
