...
class UAVParams:
    def __init__(self):
        ...
        self.nu = 6  # u=[Fx,Fy,Fz,τx,τy,τz] 单位：推力：N；力矩：N·m
...
def build_acados_dynamics_model(uav_params):
    ...
    tau_B = u[3:6]    # 机体系期望力矩 τx,τy,τz（单位：N·m）
...
class UAVHostController:
    ...
    def generate_reference_trajectory(self, x_target):
        Np = self.nmpc_params.Np
        Ts = self.nmpc_params.Ts
        x_ref = np.zeros((12, Np + 1))

        x_current = self.x_current.copy() if self.x_current is not None else np.zeros(12)

        x_decay = np.linspace(x_current[0], x_target[0], Np+1)
        y_decay = np.linspace(x_current[1], x_target[1], Np+1)      
        z_decay = np.linspace(x_current[2], x_target[2], Np+1)
        vx_decay = np.linspace(x_current[3], x_target[3], Np+1)
        vy_decay = np.linspace(x_current[4], x_target[4], Np+1)
        vz_decay = np.linspace(x_current[5], x_target[5], Np+1)
        roll_decay = np.linspace(x_current[6], x_target[6], Np+1)
        pitch_decay = np.linspace(x_current[7], x_target[7], Np+1)
        yaw_decay = np.linspace(x_current[8], x_target[8], Np+1)
        p_decay = np.linspace(x_current[7], x_target[7], Np+1)
        q_decay = np.linspace(x_current[8], x_target[8], Np+1)
        r_decay = np.linspace(x_current[8], x_target[8], Np+1)

        for i in range(Np + 1):
            ref_state = np.array([x_decay[i], y_decay[i], z_decay[i], vx_decay[i], vy_decay[i], vz_decay[i], roll_decay[i], pitch_decay[i], yaw_decay[i], p_decay[i], q_decay[i], r_decay[i]])
            x_ref[:, i] = ref_state
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            x_target = np.array([self.ref_pos_hover[0], self.ref_pos_hover[1], self.ref_pos_hover[2], 0, 0, 0, 0, 0, self.ref_yaw_hover, 0, 0, 0])
            x_ref = self.generate_reference_trajectory(x_target)
