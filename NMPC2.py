...
class UAVParams:
    def __init__(self):
        ...
        # 控制输入u = [fx,fy,fz,τx,τy,τz]  单位：N和弧度
        self.u_min = np.array([-10, -10, 0, -5, -5, -5])
...
class NMPCParams:
    def __init__(self):
        ...
        hover_thrust = -1.66 * 9.81  #无人机重力,单位N,负号表示推力方向向上
...
def build_dynamics_model(uav_params):
    ...
    nu = 6   # 控制量维度（文档定义u∈R^6）c:\Users\X\Desktop\project\QTR-NMPC\NMPC2.py
    x = ca.SX.sym('x', nx)
    u = ca.SX.sym('u', nu)
    ...
    # 控制输入向量 u = [fx,fy,fz, τx,τy,τz] 其中fx,fy,fz是机体轴向的总推力，τx,τy,τz是绕机体轴的总扭矩
    F_B = u[0:3]  # 机体轴向总推力 F_B = [fx, fy, fz]
    tau_B = u[3:6]  # 机体轴向总扭矩 τ_B = [τx, τy, τz]
    ...
    # 转动运动学：机体角速度导数
    domega_dt = ca.inv(uav_params.I) @ (-ca.cross(omega, uav_params.I @ omega) + tau_B)
    # 状态导数dx/dt
