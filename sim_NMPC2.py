...
class UAVHostController:
    ...
    def plot_results(self):
        ...
        plt.title('Desired Thrust_mumps')
        plt.xlabel('Time (s)')
        plt.ylabel('Thrust (N)')
        plt.legend()
        ...
        plt.title('Desired Torque_mumps')
        plt.xlabel('Time (s)')
        plt.ylabel('Torque (N·m)')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        save_path = os.path.join(save_dir, 'thrust_torque_mumps.png')
        ...
        ax.set_ylabel('Y (m)')
        ax.set_zlabel('Z (m)')
        ax.set_title('3D Flight Trajectory_mumps')
        ax.legend(loc='best')
        # 优化3D视角
        ax.view_init(elev=20, azim=45)
        plt.tight_layout()
        save_path = os.path.join(save_dir, '3d_trajectory_mumps.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
